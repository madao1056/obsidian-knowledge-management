#!/usr/bin/env python3
"""
Secondaryアカウントのファイルを自動的に処理するスクリプト
"""
import os
import sys
import time
...
# 設定
YOSHINANI_SCRIPT = "/Users/hashiguchimasaki/project/obsidian/100_cursor/notion_secondary_yoshinani.py"
REGULAR_SCRIPT = "/Users/hashiguchimasaki/project/obsidian/100_cursor/notion_secondary_fetcher.py"
VENV_ACTIVATE = "/Users/hashiguchimasaki/project/obsidian/notion_sync_env/bin/activate"
LOG_FILE = "/Users/hashiguchimasaki/project/obsidian/100_cursor/secondary_sync.log"
MAX_ITERATIONS = 100  # 最大実行回数
...
def run_batch_sync(script_path):
    """バッチ同期を実行"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # よしなに関連ファイル数を抽出（よしなにスクリプトの場合）
            yoshinani_count = 0
            if "yoshinani" in script_path:
                for line in output.split('\n'):
                    if "Yoshinani-related files:" in line:
                        try:
                            yoshinani_count = int(line.split("Yoshinani-related files:")[1].strip())
                        except:
                            pass
            
            return True, remaining, success, failed, yoshinani_count, output
        else:
            error_msg = f"Error: {result.stderr}"
            return False, None, 0, 0, 0, error_msg
            
    except subprocess.TimeoutExpired:
        return False, None, 0, 0, 0, "Timeout expired"
...
def main():
    """メイン処理"""
    log_message("=" * 60)
    log_message("Auto Secondary Sync Started")
    log_message("=" * 60)
    ...
    yoshinani_phase = True  # よしなに優先フェーズから開始
    ...
    while iteration < MAX_ITERATIONS:
        ...
        # 使用するスクリプトを決定
        if yoshinani_phase:
            script = YOSHINANI_SCRIPT
            log_message(f"\nIteration {iteration} starting... (Yoshinani priority mode)")
        else:
            script = REGULAR_SCRIPT
            log_message(f"\nIteration {iteration} starting... (Regular mode)")
        
        # バッチ同期を実行
        success, remaining, batch_success, batch_failed, yoshinani_count, output = run_batch_sync(script)
        ...
        # よしなに優先フェーズの終了判定
        if yoshinani_phase and yoshinani_count == 0:
            log_message("\nAll Yoshinani-related files processed!")
            yoshinani_phase = False
            log_message("Switching to regular mode...")
        elif yoshinani_phase:
            log_message(f"Yoshinani-related files remaining: {yoshinani_count}")
