#!/usr/bin/env python3
"""
Secondaryアカウントのよしなに関連ファイルを優先的に取得
"""
import os
import json
import sys
...
# Secondaryアカウント用の設定
NOTION_TOKEN = "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"
...
# Secondaryフォルダのみを対象
TARGET_DIR = "/Users/hashiguchimasaki/project/obsidian/20_Literature/25_Notion/Secondary"
...
BATCH_SIZE = 20
STATE_FILE = "/Users/hashiguchimasaki/project/obsidian/100_cursor/.secondary_yoshinani_state.json"
...
def main():
    """メイン処理"""
    print("=" * 60)
    print("Notion Secondary Yoshinani Priority Fetcher")
    print("=" * 60)
    ...
    print(f"Found {len(all_files)} placeholder files")
    ...
    for i, (filepath, notion_id, score, filename) in enumerate(batch_files):
        if score > 0:
            print(f"\n[{i+1}/{len(batch_files)}] {filename} 🎯 (Score: {score})")
        else:
