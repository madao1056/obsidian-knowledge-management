...
# 設定
SCRIPT_PATH = "/Users/hashiguchimasaki/project/obsidian/100_cursor/notion_yoshinani_main_focus.py"
VENV_ACTIVATE = "/Users/hashiguchimasaki/project/obsidian/notion_sync_env/bin/activate"
LOG_FILE = "/Users/hashiguchimasaki/project/obsidian/100_cursor/yoshinani_sync.log"
MAX_ITERATIONS = 50  # 最大実行回数
...
def main():
    """メイン処理"""
    log_message("=" * 60)
    log_message("Auto Yoshinani Sync Started")
    log_message("=" * 60)
    ...
    while iteration < MAX_ITERATIONS:
        ...
        # バッチ同期を実行
        success, remaining, batch_success, batch_failed, yoshinani_count, output = run_batch_sync()
        ...
        # よしなに関連ファイルがすべて処理されたらチェック
        if yoshinani_count == 0 and remaining > 0:
            log_message("\nAll Yoshinani-related files processed!")
            log_message(f"Remaining non-Yoshinani files: {remaining}")
            
            # ユーザーに確認
            print("\nよしなに関連ファイルの処理が完了しました。")
            print(f"残り{remaining}個の通常ファイルがあります。")
            print("続けて通常ファイルも処理しますか？ (y/n): ", end='', flush=True)
            
            try:
                response = input().strip().lower()
                if response != 'y':
                    log_message("User chose to stop after Yoshinani files.")
                    break
            except:
                # 入力エラーの場合は続行
                log_message("Continuing with regular files...")
